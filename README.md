# From Perceptual Filling-In to Stable Active Vision  

<p align="center">
  <img src="figures/Project_Overview_Diagram.png" alt="Project Overview Diagram" width="800">
</p>
---

## Overview  
This repository accompanies my academic seminar work and presentation on biologically inspired visual perception models.  
It includes a literature-based analysis (papers 1–4) and an applied experiment based on **Model #5 (ConvNet5)** from *Cohen-Duwek et al., 2021* exploring architectural and parameter sensitivity in Laplacian-based image reconstruction.  

 *Supervisor: Dr. Hadar Cohen-Duwek | The Open University of Israel | September 2025*
---

## Research Aim  
Developing **computational models inspired by human visual perception** and **neuromorphic vision**, combining biologically plausible components such as **Spiking Neural Networks (SNNs)** and **event cameras**.  
The goal is to bridge the gap between **limited, dynamic retinal input** and the **stable, complete visual experience** observed in human perception.  

---

## Repository Contents  

The repository includes:

 - ** Full written seminar report (PDF)** summarizing four key studies (2020–2024).  (seminar_paper)[link]
 - **Applied experiment** implementation extending *Model #5 (ConvNet5)* from *Cohen-Duwek et al., 2021*.  (implementation)[https://github.com/Frutta111/Neuromorphic_Vision_Seminar/blob/main/implementation/implementation.md]
 - **Final presentation slides** presentation slides and visual figures.  (presentation)[link]
-  **Reference** of the four key studies (2020–2024). (reference) [https://github.com/Frutta111/Neuromorphic_Vision_Seminar/blob/main/references/links_to_original_papers.md]

---

\## External Resources

\- \[Dataset – N-Caltech101 Event Camera](https://github.com/NBELab/CVPR-2021-W/tree/main?tab=readme-ov-file#dataset)  

\- \[NBELab – Neuromorphic \& Biologically Inspired Engineering Lab](https://github.com/NBELab)  

---

\## Citation

If you use or reference this work, please cite:

> Friedrich, E. (2025). \*From Perceptual Filling-In to Stable Active Vision.\* Advanced Seminar in Computer Science, The Open University of Israel.  

> Supervisor: Dr. Hadar Cohen-Duwek.



---




